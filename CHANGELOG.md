...
- **ドキュメントを実装に一致**: `architecture.md` のメモリ表（`DedupFilter` 8B/スロット、`NankaiPageBuffer` 28B + `MAX_PAGES × 18 + 1`、定義テーブル約 248KB（全カテゴリ・日英ラベル有効時、`-O2 -fdata-sections` ホスト実測 253,568B / 39テーブル、AVR プリセットは 3,520B））、`library.properties` の EWSS CAMF 版数（v1.1 → v1.2）、`developer-guide.md` の定義ファイル数（103 → 104）とテスト構成、`getting-started.md` の参照先と設定マクロ表、`README.md` の C++17 要件を修正した。
