...
TEST_CASE("NankaiPageBuffer isContiguous") {
    NankaiPageBuffer buffer;
    uint32_t now = currentMillis();
    
    SUBCASE("Empty buffer is not contiguous") {
        CHECK_FALSE(buffer.isContiguous());
    }
    
    SUBCASE("Single page complete is contiguous") {
        uint8_t text[18] = {'T', 'e', 's', 't', 0};
        buffer.addPage(1, 1, text, now);
        CHECK(buffer.isContiguous());
    }
    
    SUBCASE("Partial contiguous pages are contiguous") {
        uint8_t text[18] = {'T', 'e', 's', 't', 0};
        buffer.addPage(1, 3, text, now);
        buffer.addPage(2, 3, text, now);
        // Bits 0 and 1 set (pages 1 and 2 received)
        CHECK(buffer.isContiguous());  // pages 1,2 are contiguous
    }
    
    SUBCASE("Non-contiguous pages are not contiguous") {
        uint8_t text[18] = {'T', 'e', 's', 't', 0};
        buffer.addPage(1, 3, text, now);
        buffer.addPage(3, 3, text, now);
        // Bits 0 and 2 set (pages 1 and 3 received, page 2 missing)
        CHECK_FALSE(buffer.isContiguous());
    }
}

TEST_CASE("NankaiPageBuffer bitmap operations") {
    NankaiPageBuffer buffer;
    uint32_t now = currentMillis();
    uint8_t text[18] = {'T', 'e', 's', 't', 0};
    
    SUBCASE("isPageReceived after add") {
        CHECK_FALSE(buffer.isPageReceived(1));
        buffer.addPage(1, 3, text, now);
        CHECK(buffer.isPageReceived(1));
        CHECK_FALSE(buffer.isPageReceived(2));
    }
    
    SUBCASE("Bitmap tracks individual pages") {
        buffer.addPage(1, 3, text, now);
        buffer.addPage(3, 3, text, now);
        
        CHECK(buffer.isPageReceived(1));
        CHECK_FALSE(buffer.isPageReceived(2));
        CHECK(buffer.isPageReceived(3));
        
        // Verify bitmap value: bits 0 and 2 set
        uint64_t expected_bm = (1ULL << 0) | (1ULL << 2);
        CHECK(buffer.received_bitmap == expected_bm);
        CHECK(buffer.received_count == 2);
    }
    
    SUBCASE("isPageReceived out of range") {
        CHECK_FALSE(buffer.isPageReceived(0));
        CHECK_FALSE(buffer.isPageReceived(64));
        CHECK_FALSE(buffer.isPageReceived(99));
    }
}
