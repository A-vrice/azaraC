...
# Nankai scenario (azarashi tests/test.py::test_scenario3), 27 messages = 27 pages.
NANKAI_NMEAS = [
    ...
    "$QZQSM,58,C6AFA19C918002F1C0C9F1C0D471C0C673475472D74D71C0CA8ED8118DCA59C*7E",
    "$QZQSM,58,9AAFA19C918002F3CCDD73CA4FF1C0CBF1C0D371C0C271C1458CD81215E2B50*08",
    "$QZQSM,58,9AAFA19C918002F1C0C7F1C14571C0CCF1C0DCF1C14571C0C60AD812CB85F9C*06",
    "$QZQSM,58,53AFA19C918002F1C0D7F3CAD871C0D571C145F1C14371C0D188D81363A8A1C*7E",
    "$QZQSM,58,C6AFA19C918002F2D141F3CAC671C0D3F4C04D72DC5C71C0D406D813ECF2998*05",
...
def gen_nankai(data_dir):
    """Decode the Nankai scenario and emit both artifacts from one source.

    Writes nankai_vectors.json (azarashi reference) and
    test/data/nankai_pages_generated.h (C++ fixture derived from the same JSON),
    so the C++ page data can never drift from azarashi.
    """
    ...
    for r in results:
        p = r.get("params")
        if not p:
            raise SystemExit(f"Nankai decode failed: {r.get('nmea')} -> {r.get('error')}")
        raw = p["text_information"]
        pages[p["page_number"]] = raw if isinstance(raw, (bytes, bytearray)) else bytes.fromhex(raw)

    total = results[0]["params"]["total_page"]
    missing = [n for n in range(1, total + 1) if n not in pages]
    if missing:
        raise SystemExit(f"Nankai scenario is missing pages: {missing}")

    with open(os.path.join(data_dir, 'nankai_vectors.json'), 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2, default=json_serial)
    print(f"  {len(results)} Nankai messages -> nankai_vectors.json")

    header = os.path.join(data_dir, 'nankai_pages_generated.h')
    with open(header, 'w', encoding='utf-8', newline='\n') as f:
        f.write("// AUTO-GENERATED by test/scripts/gen_all_vectors.py (gen_nankai) from azarashi.\n")
        f.write("// Source of truth: test/data/nankai_vectors.json. Do not edit by hand.\n")
        f.write("#pragma once\n#include <cstdint>\n\n")
        f.write(f"static const uint8_t nankai_page_data[{total}][18] = {{\n")
        for n in range(1, total + 1):
            data = pages[n]
            if len(data) > 18:
                raise SystemExit(f"page {n}: text longer than 18 bytes ({len(data)})")
            row = data + bytes(18 - len(data))
            f.write("    { " + ", ".join(f"0x{b:02X}" for b in row) + f" }},  // page {n}\n")
        f.write("};\n")
    print(f"  {total} pages -> nankai_pages_generated.h")
